...
public class BillingServiceImpl implements BillingService {
    private final BillingDAO dao = new BillingDAOImpl();
    private final ItemDAO itemDAO = new ItemDAOImpl(); // Add ItemDAO instance
    ...
    @Override
    public Long save(Bill bill, List<BillItem> items) {
        // After saving the bill, reduce inventory for each item sold
        Long billId = dao.saveBill(bill, items);

        // Update inventory after the bill is saved
        for (BillItem item : items) {
            itemDAO.updateItemQuantity(item.getItemId(), item.getQuantity());
        }
