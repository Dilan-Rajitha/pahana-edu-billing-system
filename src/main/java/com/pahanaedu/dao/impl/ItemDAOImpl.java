...
public class ItemDAOImpl implements ItemDAO {
    ...
    @Override
    public void updateItemQuantity(Long itemId, Integer quantitySold) {
        ...
        try (Connection con = cf.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, quantitySold); // Deduct the sold quantity
            ps.setLong(2, itemId); // Item ID to identify the item in the inventory
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException("Failed to update inventory for item: " + itemId, e);
        }
    }

    // Method to count all items in the inventory
    @Override
    public int countAll() {
        String sql = "SELECT COUNT(*) FROM items";
        try (Connection con = cf.getConnection();
             ...
             ResultSet rs = st.executeQuery(sql)) {
            rs.next();
            return rs.getInt(1); // Return the count from the result set
        } catch (SQLException e) {
            throw new RuntimeException("Item countAll failed", e);
        }
    }
