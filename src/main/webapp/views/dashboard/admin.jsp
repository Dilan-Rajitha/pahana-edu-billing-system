...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .stat{
      background:var(--brand); color:#fff; border-radius:14px; padding:16px 18px;
      box-shadow:var(--shadow); display:flex; flex-direction:column; gap:10px; min-height:100px;
    }
    ...
    .tile{
      ...
      min-height:160px; padding:18px; display:flex; align-items:flex-end; justify-content:flex-start;
      position:relative; overflow:hidden; transition:.2s;
    }
    .tile:hover{transform:translateY(-2px); box-shadow:var(--shadow-lg)}
    .tile .label{color:#0b7b90; font-weight:800}
    .tile::before{
      content:""; position:absolute; inset:auto -20% 40% auto; width:140px; height:140px; background:rgba(14,180,210,.15);
      border-radius:50%; filter:blur(2px);
    }
    .tile .btn{margin-top:10px; display:inline-block; background:var(--brand); color:#fff; border:none;
...
<body>
  ...
  <div class="wrap">
    <!-- Stats row: Today’s Sales, Items Total, Registered Customers -->
    <div class="stats">
      <div class="stat">
        <h4>Today’s Sales (Rs.)</h4>
        <div class="value">
          <fmt:formatNumber value="${todaySales != null ? todaySales : 0}" type="number" minFractionDigits="2" maxFractionDigits="2"/>
        </div>
      </div>
      <div class="stat">
        <h4>Items Total :</h4>
    ...
    <div class="tiles">
      <div class="tile">
        <div>
          <div class="label">Manage Users (Admin/Staff)</div>
          <a class="btn" href="${pageContext.request.contextPath}/admin/users">Open</a>
        </div>
      </div>

      <div class="tile">
        <div>
          <div class="label">View Logs</div>
          <a class="btn" href="${pageContext.request.contextPath}/logs">Open</a>
        </div>
      </div>
      ...
      <div class="tile">
        <div>
          <div class="label">Reports (PDF)</div>
          <a class="btn" href="${pageContext.request.contextPath}/reports?dateRange=today">Open</a>
        </div>
      </div>

      <div class="tile">
        <div>
          <div class="label">View Customers</div>
          <a class="btn" href="${pageContext.request.contextPath}/customers">Open</a>
        </div>
      </div>

      <!-- NEW: Help & Support tile -->
      <div class="tile">
        <div>
